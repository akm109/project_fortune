...
## [/codeblock]
func change_scene(scene_name: String, scene_type: String, folder: String = _defaults["ScenesFolder"]) -> void:
	if folder == "/root":
		get_tree().change_scene_to_file("res://{scene}.tscn".format({"scene": scene_name}))
	else:
		get_tree().change_scene_to_file("res://{folder}/{scene_type}/{scene}.tscn".format({"folder": folder,"scene_type":scene_type,"scene": scene_name}))
